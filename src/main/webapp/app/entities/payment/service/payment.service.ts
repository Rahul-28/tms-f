...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'transactionDate'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      transactionDate: payment.transactionDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      transactionDate: restPayment.transactionDate ? dayjs(restPayment.transactionDate) : undefined,
    };
  }
