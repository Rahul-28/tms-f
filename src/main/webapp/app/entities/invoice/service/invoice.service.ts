...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'invoiceDate'> & {
...
export class InvoiceService {
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      invoiceDate: invoice.invoiceDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      invoiceDate: restInvoice.invoiceDate ? dayjs(restInvoice.invoiceDate) : undefined,
    };
  }
